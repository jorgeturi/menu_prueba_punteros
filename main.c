...
void transponer_matriz(float **, float **, int, int);
    ...
    switch (opcion_elegida)                //segun se haya tocado, pasan los casos
{
    ...
    case '1':
        ...
        matriz=(float**)malloc(N_COLUMNAS*sizeof(float*));//no se por que esto tiene que ir antes
    ...
    case '3':
        ...
        reserva_memoria_para_matriz(matrizt,N_COLUMNAST,N_FILAST);
        comprobar_reserva_matriz(matrizt, N_COLUMNAST, N_FILAST);
         transponer_matriz(matriz, matrizt, N_COLUMNAS, N_FILAS);
         imprimir_matriz(matrizt, N_COLUMNAST, N_FILAST);
        tecla_para_continuar();
        borrar_pantalla();
        break;
    ...
    case '4':
        ...
        {
            printf("\n no es posible calcular su adjunta \n");
        }
        tecla_para_continuar();
        borrar_pantalla();
        break;
    ...
    case '5':
        if (matriz_cuadrada == 0)
        {
            printf("\n no es posible calcular su determinante \n");
        }
        tecla_para_continuar();
        borrar_pantalla();
        break;
    ...
    case '6':
         if (matriz_cuadrada == 0)
       {
        printf("\n no es posible calcular su inversa \n");
       }
        tecla_para_continuar();
        borrar_pantalla();
        break;
